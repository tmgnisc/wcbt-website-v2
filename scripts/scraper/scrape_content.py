...
Usage (from the project root):
  ...
  python3 scripts/scraper/scrape_content.py            # render every route, then extract  (npm run scrape)
  python3 scripts/scraper/scrape_content.py --offline  # re-extract from content/raw/*.html only
Output: content/raw/<slug>.html and content/<slug>.json, which the Next.js app renders.
Keep ROUTES in sync with PAGES in src/config/navigation.ts.
"""
import json
import re
import subprocess
